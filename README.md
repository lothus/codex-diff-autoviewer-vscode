...
For **Codex CLI**, start `codex` in a new VS Code integrated terminal. That terminal inherits `CODEX_AUTO_OPEN_BRIDGE_FILE`, which routes edits to its owning window, including when another window has the same workspace open. A CLI session outside VS Code has no descriptor and does not open tabs. After a VS Code workspace change, start another new terminal.
